    ...
    def __init__(self, params, vocab_size, num_chars, W_init, embed_dim, num_cand,
        ...
        with self.graph.as_default():
            ...
            for i in range(K):
                ...
                if i<K-1: doc_emb = tf.nn.dropout(doc_emb, self.keep_prob)
                # qry
                indim = self.embed_dim if i==0 else 2*self.nhidden
                fgru = GRU(indim, self.nhidden, "qryfgru%d"%i)
                bgru = GRU(indim, self.nhidden, "qrybgru%d"%i, reverse=True)
                fout = fgru.compute(None, qry_emb, self.qmask) # B x N x Dh
                bout = bgru.compute(None, qry_emb, self.qmask) # B x N x Dh
                qry_emb = tf.concat([fout, bout], axis=2) # B x N x 2Dh
            # attention sum
            if cloze:
