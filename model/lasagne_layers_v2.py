...
from lasagne.layers import helper, get_output, get_all_params
...
class CorefGRULayer(MergeLayer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        mask = None
        hid_init = None
        hid_init_slow = self.hid_init_slow.copy()
        ...
        hid_init_slow = T.tile(hid_init_slow, (num_batch, 1, 1))
...
if __name__=="__main__":
    ...
    # test
    f = theano.function([x,y,m], get_output(l_cgru), on_unused_input='ignore')
    print("Expected output:")
    print(H)
    print("Actual output:")
    out = f(X[np.newaxis,:,:],Y[np.newaxis,:],M[np.newaxis,:])
    print(out)
    ...
    l_gru = GRULayer(l_in_x, 4,
            ...
            hidden_update=h,
            mask_input= l_in_m)
    f_cgru = theano.function([x,y,m], get_output(l_cgru), on_unused_input='ignore')
    f_gru = theano.function([x,y,m], get_output(l_gru), on_unused_input='ignore')
    out_cgru = f_cgru(X[np.newaxis,:,:],Y[np.newaxis,:],M[np.newaxis,:])
