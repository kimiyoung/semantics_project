...
class Model:
    ...
    def build_network(self, K, vocab_size, W_init):
        ...
        M = T.batched_dot(d, q.dimshuffle((0,2,1))) # B x N x Q
        betas = T.nnet.softmax(T.reshape(M, (M.shape[0]*M.shape[1],M.shape[2])))
        betas_r = T.reshape(betas, (M.shape[0],M.shape[1],M.shape[2]))* \
                self.inps[7][:,np.newaxis,:] # B x N x Q
        betas_r = betas_r/betas_r.sum(axis=2)[:,:,np.newaxis] # B x N x Q
        beta = betas_r.mean(axis=1) # B x Q
        alphas = T.nnet.softmax(T.reshape(M.dimshuffle((0,2,1)), 
            (M.shape[0]*M.shape[2],M.shape[1]))) # BQ x N
        alphas_r = T.reshape(alphas, (M.shape[0],M.shape[2],M.shape[1]))* \
                self.inps[6][:,np.newaxis,:] # B x Q x N
        alphas_r = alphas_r/alphas_r.sum(axis=2)[:,:,np.newaxis] # B x Q x N
        p = T.batched_dot(alphas_r.dimshuffle(0,2,1),beta) # B x N
        ...
        M = T.batched_dot(dv, q.dimshuffle((0,2,1))) # B x N x Q
        betas = T.nnet.softmax(T.reshape(M, (M.shape[0]*M.shape[1],M.shape[2])))
        betas_r = T.reshape(betas, (M.shape[0],M.shape[1],M.shape[2]))* \
                self.inps[7][:,np.newaxis,:] # B x N x Q
        betas_r = betas_r/betas_r.sum(axis=2)[:,:,np.newaxis] # B x N x Q
